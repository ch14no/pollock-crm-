...
- **AD契・NDA締結管理**: 新テーブルは作らず`deal_seller_conditions`に4列追加（`ad_contract_status`/`ad_contract_date`/`nda_status`/`nda_date`）。「締結可否」の正確な意味は酒田さん未回答のため、既存の`loss_deficit_ok`と同型（可/否、未設定=未確認）で実装（状態を増やしたくなってもCHECK差し替えのみで対応可能）。
...
**残タスク**: フェーズ4（商談記録フォーム変更: 開始/終了日時・顧客属性選択・メモ欄拡張）は未着手、ユーザーの指示待ち。酒田さんへの確認事項（フェーズ1〜3共通）: フリガナの入力必須範囲・AD契/NDA「締結可否」欄の正確な意味・ネームクリア可否の選択肢・業種分類の完全版データ投入時期・買手打診リストと上場区分等の閲覧範囲（現状は事業部限定＋super_adminに設定、全社公開が必要なら要変更）。
...
**残タスク（2026-08-28時点で解消）**: フェーズ2（買手打診結果一覧・CSV出力）・フェーズ3（AD契/NDA締結管理）は同日中に実装・デプロイ済み（上の2026-08-28(2)セクション参照）。フェーズ4（商談記録フォーム変更）のみ引き続き未着手。
