...
## 残タスク・待ち（2026-09-03時点で整理）
- **実機確認待ち**: 松木紅さん・齋藤香奈さんに、2026-07-31修正（リアルタイム同期・個人ビュー列絞り込み）が意図通り動くか確認してもらう。
- **実機確認待ち**: M&Aフェーズ5（2026-09-03、商談画面の改善7項目）——特にステージ管理のマネージャー権限アクセス・対応期日の対応済みチェック・希望価額の数値入力・「案件」表記——をユーザー側で未確認。
...
（036・037・realtime-task-syncマージ関連の旧項目・石川さん/齋藤さんの2026-07-24〜27の実機確認依頼は解消済みのため削除。当時の経緯は本ファイルの該当セクションおよびgit historyを参照）
