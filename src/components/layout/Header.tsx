...
export function Header() {
  ...
  // 設定ページで保存した通知設定（トスアップ/ステージ変更のON/OFF）を反映する
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      setNotifPrefs(loadNotifSettings())
    }, 0)
  ...
  const loadNotifications = async () => {
    ...
    // 毎ポーリング時に設定を読み直す（設定ページで変更した直後から反映させる）。
    // OFFの種別はクエリ自体を発行しない（60秒ごとの無駄なリクエストを避ける）
    const prefs = loadNotifSettings()
    setNotifPrefs(prefs)
    if (!prefs.tossup && !prefs.dealStage) return
    const since = new Date(Date.now() - NOTIF_WINDOW_MS).toISOString()
    try {
      const [count, changes] = await Promise.all([
        prefs.tossup ? fetchUnreadTossupCount(activeDivision.id) : Promise.resolve(0),
        prefs.dealStage ? fetchRecentDealStageChanges(activeDivision.id, since, currentUser?.id) : Promise.resolve([]),
      ])
  ...
  // 通知設定でOFFにした種別は件数・一覧の両方から除外する
  const visibleUnreadCount  = notifPrefs.tossup ? unreadCount : 0
  const visibleStageChanges = notifPrefs.dealStage ? stageChanges : []

  // 未読バッジ = 未読トスアップ + 前回パネルを開いて以降のステージ変更
  const newStageChangeCount = visibleStageChanges.filter(
    (a) => new Date(a.action_date).getTime() > lastSeen
  ).length
  const badgeCount = visibleUnreadCount + newStageChangeCount

  const handleOpenNotif = () => {
    setNotifOpen((o) => {
      const next = !o
      if (next) {
        // 開いた時点で最新化し、ステージ変更は既読扱いにする（設定はloadNotifications内で読み直す）
        loadNotifications()
        const now = Date.now()
        setLastSeen(now)
    ...
    <header className="hidden md:flex items-center h-16 px-6 bg-white border-b border-gray-200 sticky top-0 z-20 gap-4">
      ...
      <div className="relative flex-1 max-w-96">
        ...
        <input
          type="text"
          aria-label="顧客を検索"
          placeholder="顧客を検索...（現在の事業部・Enterで検索）"
          value={search}
        ...
        {notifOpen && (
          <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              ...
              <button onClick={() => setNotifOpen(false)} aria-label="通知を閉じる" className="text-gray-400 hover:text-gray-600">
                <X size={15} />
              </button>
            </div>

            {visibleUnreadCount === 0 && visibleStageChanges.length === 0 ? (
                ...
                {visibleUnreadCount > 0 && (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">{visibleUnreadCount}件の未読があります</p>
                    </div>
