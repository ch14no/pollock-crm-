  ...
  const loadDeals = async () => {
    ...
    try {
      ...
      if (requestSeq.current === seq) {
        toast.error(`${dealTerm}の読み込みに失敗しました。再読み込みしてください`)
      }
    } finally {
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-black text-gray-800">{dealTerm}カンバン</h1>
        ...
        {isOwnDivision ? (
          <Button icon={<Plus size={16} />} onClick={() => openDealModal()}>新規{dealTerm}</Button>
        ) : (
      ...
      {!isOwnDivision && (
        <div className="flex items-center gap-2 px-4 py-3 mb-4 bg-yellow-50 border border-yellow-200 rounded-xl text-sm text-yellow-800">
          ...
          <span>
            <strong>{activeDivision?.name}</strong> の{dealTerm}を閲覧中です。カードの移動・編集は担当事業部のみ可能です。
          </span>
        </div>
      )}
